...
export class HttpRequest {
    ...
    protected _listeners: {} = Object.create({});
    protected _onreadystatechangeCallback: any;
    protected _params: {} = Object.create({});
    ...
    protected beforeOpen(method: string, url: string, async: boolean = true, user: string = null, password: string = null) {
        ...
        switch (this.getMethod()) {
            ...
            case HttpRequest.GET:
                let _p = this.getData() || this._encodeParams();
                if (_p.length > 0) {
                    url += '?' + _p;
                }
                break;
    ...
    protected afterSend(request: any) {
        this.reset();
        return request;
    }
    setHttpListener(name: string, listener: HttpListener, once: boolean = false): void {
        this._listeners[name] = { 'listener': listener, 'once': once, 'consumed': false }
    }
    getHttpListener(name: string): HttpListener {
        if (this._listeners.hasOwnProperty(name)) {
            return this._listeners[name].listener;
        }
        return null;
    }
    removeHttpListener(name: string) {
        if (this._listeners.hasOwnProperty(name)) {
    ...
    protected _resetListener() {
        ...
        for (let n in this._listeners) {
            if (this._listeners.hasOwnProperty(n)) {
                _r.addEventListener('timeout', function(e) {
                    if(_this._listeners[n].once && _this._listeners[n].consumed)return;
                    _this._listeners[n].listener.onTimeout(e, new HttpResponse(this,n));
                }, false);
                _r.addEventListener('abort', function(e) {
                    if(_this._listeners[n].once && _this._listeners[n].consumed)return;
                    _this._listeners[n].listener.onAbort(e, new HttpResponse(this,n));
                }, false);
                _r.addEventListener('error', function(e) {
                    if(_this._listeners[n].once && _this._listeners[n].consumed)return;
                    _this._listeners[n].listener.onError(e, new HttpResponse(this,n));
                }, false);
                _r.addEventListener('loadstart', function(e) {
                    if(_this._listeners[n].once && _this._listeners[n].consumed)return;
                    //_this._listeners[n].consumed = false;
                    _this._listeners[n].listener.onLoadStart(e, new HttpResponse(this,n));
                }, false);
                _r.addEventListener('progress', function(e) {
                    if(_this._listeners[n].once && _this._listeners[n].consumed)return;
                    _this._listeners[n].listener.onProgress(e, new HttpResponse(this,n));
                }, false);
                _r.addEventListener('load', function(e) {
                    if(_this._listeners[n].once && _this._listeners[n].consumed)return;
                    _this._listeners[n].listener.onLoad(e, new HttpResponse(this,n));
                }, false);
                _r.addEventListener('loadend', function(e) {
                    if(_this._listeners[n].once && _this._listeners[n].consumed)return;
                    _this._listeners[n].listener.onLoadEnd(e, new HttpResponse(this,n));
